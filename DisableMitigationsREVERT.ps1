Reg Delete "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\kernel" /v "MitigationAuditOptions" /f > $null 2>&1
...
Reg Delete "HKLM\SOFTWARE\Microsoft\WindowsMitigation" /v "UserPreference" /f > $null 2>&1
Reg Delete "HKLM\SYSTEM\CurrentControlSet\Control\SCMConfig" /v "EnableSvchostMitigationPolicy" /f > $null 2>&1
...
# Enbale CFG for the system
Set-ProcessMitigation -System -Enable CFG > $null 2>&1
